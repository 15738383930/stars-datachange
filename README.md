...
ps：图中字段都可自定义（详见下一步），这里按图中字段讲解：
    1.1 code——对应属性的字典代码
    1.2 name——对应属性的字典值（也就是你要的结果）
    1.3 type——对应属性的名称（下图中，第一行数据是后几行数据的父级，用parent_id声明【parent_id、id都可自定义字段名】）
    1.4 父字典的字段——type的值，对应数据模型中@ChangeModel的modelName的值（建议用数据模型的名称，首字母小写）
```
